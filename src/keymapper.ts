...
const specialMap = {
  LGUI_T: (key: string) => `&mt LG(${key})`,
  RGUI_T: (key: string) => `&mt RG(${key})`,
  LCAG_T: (key: string) => `&mt LC(LA(LG(${key})))`,
  RCAG_T: (key: string) => `&mt RC(RA(RG(${key})))`,
  LCTL_T: (key: string) => `&mt LC(${key})`,
  RCTL_T: (key: string) => `&mt LC(${key})`,
  LALT_T: (key: string) => `&mt LA(${key})`,
  RALT_T: (key: string) => `&mt RA(${key})`,
}
